...
{
    public class GoogleDrive : IBackend_v2, IStreamingBackend, IBackendGUI
    {
        private const string AUTHID_OPTION = "google-authid";
        ...
        public GoogleDrive(string url, Dictionary<string, string> options)
        {
            ...
            //    throw new Exception(Strings.GoogleDrive.MissingAuthIDError);
        ...
        private string GetFolderId(string path, bool autocreate = false)
        {
            ...
            foreach (var p in path.Split(new char[] { '/' }, StringSplitOptions.RemoveEmptyEntries))
            {
                ...
                if (res.Length == 0)
                {
                    ...
                    throw new Exception(StringsBackport.GoogleDrive.MultipleEntries(p, curdisplay));
                else
                    curparent = res[0].id;
        ...
        public string Description => StringsBackport.GoogleDrive.Description;
        public string DisplayName => StringsBackport.GoogleDrive.DisplayName;
        public string ProtocolKey => "googledrive";

        public IList<ICommandLineArgument> SupportedCommands => 
            new List<ICommandLineArgument>(new ICommandLineArgument[] {
                    new CommandLineArgument(AUTHID_OPTION, CommandLineArgument.ArgumentType.Password, StringsBackport.GoogleDrive.AuthidShort, StringsBackport.GoogleDrive.AuthidLong(OAuthHelper.OAUTH_LOGIN_URL("googledrive"))),
                });
        ...
        public void Delete(string remotename)
        {
            try
            {
                ...
                {
                    var url = string.Format("{0}/files/{1}", DRIVE_API_URL, Uri.UrlPathEncode(fileid));
        ...
        public void Put(string remotename, System.IO.Stream stream)
        {
            try
            {
                ...
                var isUpdate = !Misc.IsNullOrWhiteSpace(fileid);

                var url = isUpdate ?
                    string.Format("{0}/files/{1}?uploadType=resumable", DRIVE_API_UPLOAD_URL, Uri.UrlPathEncode(fileid)) :
        ...
        {
            get { return GoogleDriveUI.PageTitle; }
        }

        public string PageDescription
        {
            get { return GoogleDriveUI.PageDescription; }
        }
        ...
        {
            return GoogleDriveUI.GetConfiguration(guiOptions, commandlineOptions);
        }
        #endregion
        ...
        private IEnumerable<GoogleDriveFolderItem> ListFolder(string parentfolder, bool? onlyFolders = null, string name = null)
        {
            ...
            var url = string.Format("{0}/files?q={1}", DRIVE_API_URL, Uri.UrlEncode(string.Join(" and ", p.Where(x => x != null).ToArray())));
            var token = string.Empty;

            do
            {
                var res = m_oauth.GetJSONData<GoogleDriveListResponse>(url + (Misc.IsNullOrWhiteSpace(token) ? "" : "&pageToken=" + Uri.UrlEncode(token)));
                foreach (var n in res.items)
                    yield return n;

                token = res.nextPageToken;
            } while (!Duplicati.Library.Misc.IsNullOrWhiteSpace(token));
        }
