    ...
    public struct Uri
    {
        ...
        public Uri(string url)
        {
            ...
            if (!m.Success || m.Length != url.Length)
            {
                ...
                throw new ArgumentException(Strings.UriParseError(url), "url");
            }
        ...
        public void RequireHost()
        {
            if (string.IsNullOrEmpty(Host))
                throw new ArgumentException(Strings.NoHostname(OriginalUri));
        }
        ...
        private static byte[] HexStringAsByteArray(string hex, byte[] data)
        {
        ...
        {
            public static string UriParseError(string uri) { return String.Format(@"The Uri is invalid: {0}", uri); }
            public static string NoHostname(string uri) { return String.Format(@"The Uri is missing a hostname: {0}", uri); }
        }
    }
}
