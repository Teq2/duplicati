...
namespace Duplicati.Library.Backend
{
    public class OAuthHelper : JSONWebHelper
    {
        ...
        { 
            get { return Misc.IsNullOrWhiteSpace(_override_server) ? DUPLICATI_OAUTH_SERVICE : _override_server; }
        ...
        {
            var u = new Uri(OAUTH_SERVER);
            var addr = u.SetPath("").SetQuery((u.Query ?? "") + (Misc.IsNullOrWhiteSpace(u.Query) ? "" : "&") + "type={0}");
                ...
                if (m_token == null || m_tokenExpires < DateTime.UtcNow)
                {
                    ...
                    while(true)
                    {
                        try
                        {
                            ...
                            if (!Misc.IsNullOrWhiteSpace(res.v2_authid))
                        ...
                        catch (Exception ex)
                        {
                            ...
                            if (ex is WebException)
                            {
                                ...
                                if (resp != null)
                                {
                                    ...
                                    if (Misc.IsNullOrWhiteSpace(msg))
